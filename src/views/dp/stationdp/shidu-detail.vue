<template>
  <div class="Index_bgImg">
    ...
    <div style="position: relative;top: 8rem;">
      ...
      <span style="position: absolute;left:4rem;top:1.28rem;font-size: 16px;font-weight:bold;font-family:PingFang-SC;color:rgba(255,255,255,1);">温湿度监测仪/湿度</span>
    </div>
       ...
       var that=this;
        ...
        axios.get('/GW.WIR/show/getHumidityReport.action', {
          ...
          exception.forEach((item,index)=>{
            ...
            const exceptionItem = {}
            exceptionItem.name = item.humidity
            exceptionItem.value = item.humidity
            exceptionItem.xAxis = x
            exceptionItem.yAxis = item.humidity
            exceptionList.push(exceptionItem)
          })
          this.humidityOption.series[0].markPoint.data = exceptionList
          // console.log(JSON.stringify(this.humidityOption))
        })
      },
      formatDate(now) {
