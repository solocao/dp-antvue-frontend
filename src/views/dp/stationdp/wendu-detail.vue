<template>
  <div class="Index_bgImg">
    ...
    <div style="position: relative;top: 8rem;">
      ...
      <span style="position: absolute;left:4rem;top:1.28rem;font-size: 16px;font-weight:bold;font-family:PingFang-SC;color:rgba(255,255,255,1);">温温度监测仪/温度</span>
    </div>
    <div style="position: relative;top: 13rem;">
      <img style="position: absolute;top:0.8rem;left: 2rem;" src="./img/icon6_1.png">
      <span style="position: absolute;top:1.6rem;left: 5rem;font-size: 18px;font-family:PingFang-SC;color:rgba(255,255,255,1);">历史温度趋势</span>
  ...
  export default {
    ...
    methods: {
      ...
      getTemp() {
        ...
        axios.get('/GW.WIR/show/getTemperatureReport.action', {
          ...
          exception.forEach((item, index) => {
            ...
            const exceptionItem = {}
            exceptionItem.name = item.temperature
            exceptionItem.value = item.temperature
            exceptionItem.xAxis = x
            exceptionItem.yAxis = item.temperature
            exceptionList.push(exceptionItem)
          })
          that.tempOption.series[0].markPoint.data = exceptionList
        })
      },
      formatDate(now) {
      ...
      cellClassHandle({
        ...
        if (row.temperature >= 70 && columnIndex == 1) {
          return 'exception'
        }
      }
    },
